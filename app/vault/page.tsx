...
function VaultContent() {
  ...
  const vaultBg: MessageBg = (((profile as any)?.vault_bg) ??
    'plain') as MessageBg;
  const vaultBgDark = isDarkBg(vaultBg);
  const vaultMetaColor = vaultBgDark
  ...
  const [highlight, setHighlight] = useState<{ id: string; key: number } | null>(
    null
  );
    ...
    supabase.auth.getUser().then(({ data }) => {
      ...
      if (!e) {
        ...
        supabase
          ...
          .eq('id', user!.id)
          .then(({ error }) => {
            if (error) console.error('last_seen_vault failed:', error);
          });
      }
    });
  }, []);
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (updated.sender_id !== userId && updated.recipient_id !== userId)
            return;
          setDms((prev) =>
  ...
  async function sendDm(e: React.FormEvent) {
    ...
    if (error) {
      ...
      setDms((prev) =>
        prev.map((m) => (m.id === tempId ? (data as DM) : m))
      );
    }
    setSendingDm(false);
  }
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      ...
                      <p
                        style={{
                          ...
                          fontWeight: 800,
                          color:
                            unread > 0 ? '#C2185B' : 'rgba(0,0,0,0.5)',
                        }}
                      >
                        {unread > 0
    ...
    >
      ...
      >
        ...
        <SwipeCarousel
          ...
          slides={[messagesSlide, notesSlide]}
        />
      </div>
