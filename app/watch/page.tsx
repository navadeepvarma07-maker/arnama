...
export default function WatchPage() {
  ...
  useEffect(() => {
    ...
    const i = setInterval(async () => {
      ...
      await supabase.from('watch_rooms').update({ position_seconds: t }).eq('id', 'main');
    }, 1500);
    return () => clearInterval(i);
  ...
  async function togglePlay() {
    const p = getPlayer();
    if (!p) return;
    if (!isDJ) { if (p.isPlaying()) p.pause(); else p.play(); return; }
  ...
  const videoInner = (
    <>
      {showYTPlayer && <div ref={ytContainerRef} style={{ width: '100%', height: '100%' }} />}
      {showLocalPlayer && (
        <video
          ref={videoElRef}
      ...
      {showLocalNeedFile && (
        <div style={{
          ...
          padding: '14px', textAlign: 'center', background: '#000', color: '#FFF',
        }}>
          <span style={{ fontSize: '40px' }}>📁</span>
          <p style={{ margin: 0, fontSize: '12px', fontWeight: 800 }}>DJ playing: <strong>{room?.local_hint}</strong></p>
      ...
      {showEmpty && (
        <div style={{
          ...
          background: `linear-gradient(180deg, #2a1a3e 0%, #1a0b2e 100%)`, color: '#FFF',
        }}>
      ...
      {floaters.map((f) => (
        <span key={f.id} aria-hidden style={{
          position: 'absolute', bottom: '20px', left: `${f.x}%`,
          fontSize: immersive ? '44px' : '32px', lineHeight: 1,
          pointerEvents: 'none',
          animation: 'reaction-float 2.2s ease-out forwards',
          filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.6))', zIndex: 6,
        }}>{f.emoji}</span>
      ))}
    </>
  );

  // Immersive chrome — portals for bars/drawer
    ...
    <>
      ...
      >
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
            ...
            {room?.dj_email && (
              <span style={{
                ...
                background: 'rgba(255,139,167,0.9)', color: '#000', fontSize: '10px', fontWeight: 900,
              }}>
          ...
          {!voice.error && (voice.micOn || voice.participants.length > 0) && (
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
              {email && <ImmersiveVoiceChip email={email} speaking={voice.micOn} mine muted={!voice.micOn} color={AVATAR_COLORS[0]} />}
              {voice.participants.map((p, i) => (
                <ImmersiveVoiceChip key={p.id} email={p.email} speaking={p.speaking}
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexShrink: 0 }}>
          <button onClick={() => voice.toggleMic()} disabled={!!voice.error} style={{
          ...
          <button onClick={() => setChatDrawerOpen((v) => !v)} style={{
            width: '40px', height: '40px', borderRadius: '999px', border: '2px solid #FFF',
          ...
          <button onClick={() => setImmersive(false)} style={{
            ...
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}><Minimize2 className="size-4" strokeWidth={3} /></button>
        </div>
      </div>
      ...
      <div
        ...
        style={{
          ...
          padding: '16px 20px 20px', zIndex: 10000,
          background: 'linear-gradient(0deg, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0) 100%)',
          opacity: controlsVisible ? 1 : 0, transition: 'opacity 0.25s',
          pointerEvents: controlsVisible ? 'auto' : 'none',
      ...
      >
        <div
          ...
          style={{
            height: '6px', width: '100%', background: 'rgba(255,255,255,0.25)',
            borderRadius: '999px', position: 'relative',
            cursor: duration > 0 ? 'pointer' : 'default', touchAction: 'none',
          }}
        >
          <div style={{
            height: '100%', width: `${pct}%`, background: '#FF8BA7', borderRadius: '999px',
          ...
          {duration > 0 && (
            <div style={{
              ...
              borderRadius: '999px', background: '#FFF', boxShadow: '0 2px 6px rgba(0,0,0,0.5)',
              pointerEvents: 'none',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px', flexWrap: 'wrap' }}>
          <button onClick={togglePlay} disabled={!hasVideo} style={{
            width: '52px', height: '52px', borderRadius: '999px', border: '2px solid #FFF',
            background: hasVideo ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.3)',
            color: '#000', cursor: hasVideo ? 'pointer' : 'not-allowed',
            display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
          }}>
            {isPlaying ? <Pause className="size-6" strokeWidth={3} style={{ fill: '#000' }} />
              : <Play className="size-6" strokeWidth={3} style={{ fill: '#000', marginLeft: '2px' }} />}
          </button>
          <span style={{ fontSize: '14px', fontWeight: 900, color: '#FFF', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>
      ...
      {chatDrawerOpen && (
        <div data-controls style={{
          ...
          <div style={{
            padding: '14px 16px', borderBottom: '3px solid black', background: '#E6E6FA',
            ...
            <button onClick={() => setChatDrawerOpen(false)} style={{
              width: '26px', height: '26px', borderRadius: '999px',
              border: '2px solid black', background: '#FFD1DC', color: '#000',
          ...
          <form onSubmit={sendChat} style={{
            ...
            }}><Send className="size-4" strokeWidth={2.75} /></button>
          </form>
        </div>
      )}
    </>,
      ...
      >
        ...
        <div className="flex items-center justify-between shrink-0 gap-2">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button onClick={() => setChatOpen((v) => !v)} style={{
              width: '44px', height: '44px', border: '4px solid black', borderRadius: '14px',
              background: chatOpen ? '#FF8BA7' : '#E2F0D9',
              color: '#000', fontWeight: 900, cursor: 'pointer',
              boxShadow: '4px 4px 0 0 black',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}><MessageSquare className="size-5" strokeWidth={2.75} /></button>
            <Link href="/"
              ...
              style={{ padding: '8px 12px', fontSize: '14px', minWidth: '44px', minHeight: '44px' }}>←</Link>
          </div>
        </div>
            ...
            <>
              ...
              <div
                ...
                style={{
                  ...
                  zIndex: 5, touchAction: 'none', borderTopLeftRadius: '12px',
        ...
        <div className="border-4 border-black shrink-0"
          ...
          <div
            ...
            onPointerUp={onBarUp} onPointerCancel={onBarUp}
            style={{
              height: '18px', width: '100%', background: 'rgba(0,0,0,0.15)',
              borderRadius: '999px', border: '2px solid black', position: 'relative',
              cursor: duration > 0 ? 'pointer' : 'default', touchAction: 'none',
            }}>
            <div style={{
          ...
          <div className="flex items-center gap-3 mt-3">
            <button onClick={togglePlay} disabled={!hasVideo} style={{
              width: '48px', height: '48px', borderRadius: '14px', border: '3px solid black',
              background: `linear-gradient(180deg, rgba(255,255,255,0.55) 0%, rgba(255,255,255,0) 55%), #E2F0D9`,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              cursor: hasVideo ? 'pointer' : 'not-allowed',
              boxShadow: '3px 3px 0 0 black', opacity: hasVideo ? 1 : 0.5, flexShrink: 0,
            }}>
              {isPlaying ? <Pause className="size-5" strokeWidth={3} style={{ color: '#000', fill: '#000' }} />
        ...
        {isDJ && (
          <div className="border-4 border-black shrink-0"
            ...
            {ytError && <p style={{ margin: 0, fontSize: '11px', fontWeight: 800, color: '#C2185B' }}>⚠️ {ytError}</p>}
            <button onClick={() => fileInputRef.current?.click()} style={{
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              gap: '8px', padding: '12px', border: '2px solid black', borderRadius: '12px',
        ...
        {chatOpen && (
          <div className="border-4 border-black shrink-0"
            ...
            <div style={{
              padding: '10px 14px', borderBottom: '3px solid black', background: '#E6E6FA',
              display: 'flex', alignItems: 'center', gap: '8px',
            }}>
              <MessageSquare className="size-3.5" strokeWidth={3} />
              <p style={{ margin: 0, fontSize: '12px', fontWeight: 900, color: '#000' }}>live chat · {messages.length}</p>
            </div>
            <div ref={chatScrollRef} onScroll={handleChatScroll}
...
function ImmersiveVoiceChip({ email, speaking, mine, muted, color }: {
  email: string; speaking: boolean; mine?: boolean; muted?: boolean; color: string;
}) {
  return (
    <span style={{
      display: 'inline-flex', alignItems: 'center', gap: '6px',
      padding: '3px 10px 3px 3px', border: '2px solid #FFF', borderRadius: '999px',
      background: speaking ? 'rgba(127,229,165,0.9)' : 'rgba(0,0,0,0.55)',
      color: speaking ? '#000' : '#FFF', fontSize: '11px', fontWeight: 900,
      transition: 'background 0.15s',
    }}>
      <span style={{
        width: '22px', height: '22px', borderRadius: '999px', border: '2px solid #000',
